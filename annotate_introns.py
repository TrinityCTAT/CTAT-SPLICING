...
def main():
    ...
    parser.add_argument(
        "--genome_lib_dir",
        type=str,
        default=os.environ["CTAT_GENOME_LIB"],
        required=False,
        help="CTAT genome lib",
    )
    ...
    intron_db_file = os.path.join(
        genome_lib_dir, "ctat_splicing_lib/known_intron_annotations.tsv.gz"
    )
    ...
    for chrom, chrom_icounter in intron_counter.items():
        ...
        for intron, count in sorted(chrom_icounter.items()):
            ...
            cryptic_or_known = (
                intron_db[intron_key] if intron_key in intron_db else "Cryptic"
            )
...
def capture_introns_from_file(introns_file, intron_counter):
    ...
    with open(introns_file, "rt") as fh:
        for line in fh:
            ...
            if m is None:
                raise RuntimeError(
                    "Error, couldn't parse intron from {} in chr\\d+:\\d+-\\d+ format".format(
                        line
                    )
                )
            chrom = m.group(1)
...
def get_intron_db(known_introns_file):
    ...
    with gzip.open(known_introns_file, "rt") as fh:
        ...
        assert re.match(
            "intron\tgenes", header
        ), "Error, {} lacks expected header".format(header)
